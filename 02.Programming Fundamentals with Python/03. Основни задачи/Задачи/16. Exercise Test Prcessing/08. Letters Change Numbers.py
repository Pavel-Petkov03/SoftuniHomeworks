...
# position is 2. So we add 2 to the resulted number (12+2=14). Similarly for the second string s is lowercase and
# its position is 19 so we multiply it with the number (17*19 = 323). Then we have Uppercase G with position 7, so
# we subtract it from the resulted number (323 – 7 = 316). Finally, we sum the 2 results and we get 14 + 316=330.
# Input
...
for substring in text:
	digits = int(substring[1:-1])
	if substring[0].isupper():
		first_letter_value = ord(substring[0]) - 64
		cycle_sum = digits / first_letter_value
	else:
		first_letter_value = ord(substring[0]) - 96
		cycle_sum = digits * first_letter_value

	if substring[-1].isupper():
		second_letter_value = ord(substring[-1]) - 64
		cycle_sum -= second_letter_value
	else:
		second_letter_value = ord(substring[-1]) - 96
		cycle_sum += second_letter_value
