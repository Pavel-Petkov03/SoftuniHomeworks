...
# This is our neighborhood. After that a series of Jump commands will
# follow, until you receive "Love!" Every house in the neighborhood needs
# a certain number of hearts delivered by Cupid, in order to be able to celebrate
...
# decreased by 2. If the needed hearts for a certain house become equal to 0 , print on the
# console "Place {houseIndex} has Valentine's day." If Cupid jumps to a house where the needed
# hearts are already 0, print on the console "Place {houseIndex} already had Valentine's day.".
# Keep in mind that Cupid can have a bigger jump length than the size of the neighborhood and if
# he does jump outside of it, he should start from the first house again.
# For example, we are given this neighborhood: 6@6@6. Cupid is at the start and jumps with a length
# of 2. He will end up at index 2 and decrease the needed hearts there by 2: [6, 6, 4]. Next he jumps
# again with a length of 2 and goes outside the neighborhood, so he goes back to the first house (index 0)
...
def valentine(array):
	...
	while command != "Love!":
		...
		command = input()
	filtered = list(filter(lambda x: x <= 0, array))
	print(f"Cupid's last position was {index}.")
	if len(filtered) == len(array):
		return "Mission was successful."
	else:
		return f"Cupid has failed {len(array) - len(filtered)} places."


our_array = input()
print(valentine(our_array))
