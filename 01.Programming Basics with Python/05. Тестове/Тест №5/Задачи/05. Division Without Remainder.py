...
# От тях някакъв процент p1 се делят без остатък на 2, друг
# процент p2 се делят без остатък на 3, друг процент p3 се делят без остатък
# на 4. Да се напише програма, която изчислява и отпечатва процентите p1, p2 и p3.
...
# Изход
# Да се отпечатат на конзолата 3 реда, всеки от които съдържа
# процент между 0% и 100%, с точност две цифри след десетичната точка,
...
for c in range(1 , n + 1):
    ...
    if number % 4 == 0:
        p3 += 1
print(f'{(p1 / all_counter * 100):.2f}%')
print(f'{(p2 / all_counter * 100):.2f}%')
print(f'{(p3 / all_counter * 100):.2f}%')
